...
// Import BrandStyle type for enhanced brand style support
...
NOW CREATE A BACKGROUND IMAGE:
- This is a background for social media carousel slides
- Leave clear space in the center for text overlay
- Do NOT add any text, words, letters, or numbers to the image
- The background should be suitable for overlaying ${brandStyle.colorPalette?.text || 'white'} text
- Match the brand's visual identity EXACTLY as described above
- Use the exact colors: Primary ${brandStyle.colorPalette?.primary}, Accent ${brandStyle.colorPalette?.accent}, Background ${brandStyle.colorPalette?.background}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Type the brandStyle for TypeScript (enhanced brand style system)
    ...
    // Note: "video" style is now for single video posts, not carousel
    // Only "mixed-carousel" and "video-carousel" trigger video generation in carousels
    ...
    // Determine if we should generate a background:
    // - If brand style priority is enabled, ALWAYS generate using brand style (ignore preset backgrounds)
    // - Otherwise, generate if backgroundStyle is specified
    const shouldGenerateBackground = brandStylePriorityEnabled && typedBrandStyle?.masterPrompt
      ? true // Always generate with brand style
      : !!backgroundStyle;

    if (!bgImage && shouldGenerateBackground) {
      const googleApiKey = process.env.GOOGLE_API_KEY;
      if (googleApiKey) {
        if (jobId) {
          await updateJobStatus(supabase, jobId, {
            progress: 5,
            currentStep: brandStylePriorityEnabled ? 'Generating brand-styled background' : 'Generating background',
          });
        }
